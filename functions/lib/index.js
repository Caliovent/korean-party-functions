...
exports.createGuild = exports.getGuildDetails = exports.joinGuild = exports.leaveGuild = exports.purchaseShopItem = void 0;
...
exports.leaveGuild = functions.https.onCall(async (request) => {
    if (!request.auth) {
        throw new functions.https.HttpsError("unauthenticated", "L'utilisateur doit être authentifié pour quitter une guilde.");
    }
    const { guildId } = request.data;
    const userId = request.auth.uid;
    if (!guildId || typeof guildId !== "string") {
        throw new functions.https.HttpsError("invalid-argument", "L'ID de la guilde est requis.");
    }
    const userRef = db.collection("users").doc(userId);
    ...
    try {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            if (!guildDoc.exists) {
                ...
                throw new functions.https.HttpsError("not-found", `Guilde avec l'ID "${guildId}" non trouvée. ` +
            ...
            if (!memberInfo) {
                ...
                transaction.update(userRef, { guildId: admin.firestore.FieldValue.delete() });
                throw new functions.https.HttpsError("failed-precondition", "Vous n'êtes pas listé dans les membres de la guilde. " +
                    "Votre profil a été mis à jour.");
            }
            // Prepare updates
            const userUpdate = {
        ...
        throw new functions.https.HttpsError("internal", "Une erreur interne est survenue lors de la tentative de quitter la guilde.");
    }
});
exports.joinGuild = functions.https.onCall(async (request) => {
    if (!request.auth) {
        throw new functions.https.HttpsError("unauthenticated", "L'utilisateur doit être authentifié pour rejoindre une guilde.");
    }
    const { guildId } = request.data;
    const userId = request.auth.uid;
    if (!guildId || typeof guildId !== "string") {
    ...
    try {
        await db.runTransaction(async (transaction) => {
            ...
            const userData = userDoc.data();
            if (userData?.guildId) {
                throw new functions.https.HttpsError("failed-precondition", "L'utilisateur est déjà membre d'une guilde.");
            }
            if (!guildDoc.exists) {
                throw new functions.https.HttpsError("not-found", `Guilde avec l'ID "${guildId}" non trouvée.`);
            }
            ...
            if (!guildData) { // Should not happen if guildDoc.exists is true
                throw new functions.https.HttpsError("internal", "Données de guilde non trouvées malgré l'existence du document.");
            }
            ...
            if (guildData.members && guildData.members[userId]) {
                ...
                throw new functions.https.HttpsError("already-exists", // Or "failed-precondition"
        ...
        throw new functions.https.HttpsError("internal", "Une erreur interne est survenue pour rejoindre la guilde.");
    }
});
// Potentiellement d'autres fonctions ici...
// export const anotherFunction = functions.https.onRequest(...)
exports.getGuildDetails = functions.https.onCall(async (request) => {
    if (!request.auth) {
        throw new functions.https.HttpsError("unauthenticated", "L'utilisateur doit être authentifié pour voir les détails d'une guilde.");
    }
    const { guildId } = request.data;
    const userId = request.auth.uid;
    if (!guildId || typeof guildId !== "string") {
    ...
    try {
        ...
        if (!guildData.members || !guildData.members[userId]) {
            throw new functions.https.HttpsError("permission-denied", "L'utilisateur n'est pas membre de cette guilde ou les données des membres sont manquantes.");
        }
        ...
        throw new functions.https.HttpsError("internal", "Une erreur interne est survenue lors de la récupération des détails de la guilde.");
    }
});
exports.createGuild = functions.https.onCall(async (request) => {
    if (!request.auth) {
        throw new functions.https.HttpsError("unauthenticated", "L'utilisateur doit être authentifié pour créer une guilde.");
    }
    const { name, tag, description } = request.data;
    const userId = request.auth.uid;
    if (!name || typeof name !== "string" || name.length < 3 || name.length > 50) {
        throw new functions.https.HttpsError("invalid-argument", "Le nom de la guilde doit comporter entre 3 et 50 caractères.");
    }
    if (!tag || typeof tag !== "string" || tag.length < 2 || tag.length > 5) {
        throw new functions.https.HttpsError("invalid-argument", "Le tag de la guilde doit comporter entre 2 et 5 caractères.");
    }
    if (!description || typeof description !== "string" || description.length < 10 || description.length > 250) {
        throw new functions.https.HttpsError("invalid-argument", "La description de la guilde doit comporter entre 10 et 250 caractères.");
    }
    const userRef = db.collection("users").doc(userId);
    ...
    try {
        const guildId = await db.runTransaction(async (transaction) => {
            ...
            if (!nameSnapshot.empty) {
                throw new functions.https.HttpsError("already-exists", `Une guilde avec le nom "${name}" existe déjà.`);
            }
            const tagSnapshot = await transaction.get(existingGuildByTagQuery);
            if (!tagSnapshot.empty) {
                throw new functions.https.HttpsError("already-exists", `Une guilde avec le tag "${tag}" existe déjà.`);
            }
            const userDisplayName = userData?.displayName || "Joueur Anonyme";
        ...
        throw new functions.https.HttpsError("internal", "Une erreur interne est survenue lors de la création de la guilde.");
    }
});
